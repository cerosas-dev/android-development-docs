...
Present the gap list to the user as a multi-select in a single message. Never bulk-write without confirmation.
...
- **N targets** → **dispatch one `android-test-target` subagent per target**, single message. Pass each: production path, layer, the template section, `MainDispatcherRule` path, fakes to reuse, output path.
...
- Sequential only: input prompt + shared-fake pre-writes + final `./gradlew :<module>:testDebugUnitTest`.
...
5. **For 2+ targets: dispatch one `android-test-target` subagent per target** (single message).
