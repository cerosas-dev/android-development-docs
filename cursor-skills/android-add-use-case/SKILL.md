...
- **Multiple use cases in one invocation** → dispatch one `android-feature-target` subagent per use case, single message.
- Sequential only: input prompt + base abstractions (if missing) + final `./gradlew compileDebugKotlin`.
