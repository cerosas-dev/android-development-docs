...
- **N targets** → **dispatch one `android-test-target` subagent per target**, single message. Pass each: production path, the template section, fake repo paths (pre-written), `@TestInstallIn` path (pre-written), output path.
- **Shared `Fake*Repository` + `TestRepositoryModule`**: write **once before** the fan-out; never let agents race to write them.
- Sequential only: input prompt + one-time/shared writes + final `connectedDebugAndroidTest`.
